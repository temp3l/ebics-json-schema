...
function reset(attrPrefix, laxURIprocessing, newXsPrefix, lang) {
  target = null;
...
function doElement(src, parent, key) {
  ...
  if (name && type) {
    ...
    if (doc) {
      if (doc.length) typeData.description = doc[0]['#text'];
      else {
        typeData.description = doc['#text'];
      }
    }
    ...
    if (parent[xsPrefix + 'annotation'] && parent[xsPrefix + 'annotation'][xsPrefix + 'documentation']) {
      ...
      if (desc.length) {
        target.description = desc[0]['#text'];
      } else {
        target.description = desc['#text'];
      }
    }
...
module.exports = {
  getJsonSchema: function getJsonSchema(src, title, outputAttrPrefix, laxURIs, newXsPrefix) {
