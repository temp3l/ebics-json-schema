...
function reset(attrPrefix, laxURIprocessing, newXsPrefix, _lang) {
  target = null;
  attributePrefix = attrPrefix;
  laxURIs = laxURIprocessing;
  defaultNameSpace = '';
  xsPrefix = newXsPrefix || '';
  lang = _lang || 'de';
}
...
function getDescription(doc, lang) {
  ...
  if (Array.isArray(doc)) {
    let _res;
    _res = doc.filter((d) => d['@xml:lang'] === lang);
    if (_res.length === 1) return _res[0]['#text'];
    return doc[0]['#text'];
...
function finaliseType(typeData) {
  if (typeData.type == 'string' || typeData.type == 'boolean' || typeData.type == 'array' || typeData.type == 'object' || typeData.type == 'integer' || typeData.type == 'number' || typeData.type == 'null') {
    //typeData.type = typeData.type;
  } else {
    if (typeData.type.startsWith('xml:')) {
      // id, lang, space, base, Father
...
function doElement(src, parent, key) {
  ...
  if (element['@type']) {
    ...
    if (element[xsPrefix + 'simpleType'][xsPrefix + 'restriction']) {
      type = element[xsPrefix + 'simpleType'][xsPrefix + 'restriction']['@base'];
    } else {
      type = element[xsPrefix + 'simpleType']['list'][xsPrefix + '@itemType'];
    }
  ...
  if (name && type) {
    ...
    if (doc) {
      typeData.description = getDescription(doc, lang);
    ...
    if (key == xsPrefix + 'attributeGroup' && element['@ref']) {
      // || (name == '$ref')) {
    ...
    if (parent[xsPrefix + 'annotation'] && parent[xsPrefix + 'annotation'][xsPrefix + 'documentation']) {
      let desc = parent[xsPrefix + 'annotation'][xsPrefix + 'documentation'];
      target.description = getDescription(desc, lang);
    }
    if (element[xsPrefix + 'annotation'] && element[xsPrefix + 'annotation'][xsPrefix + 'documentation']) {
      //let desc = (target.description ? target.decription + '\n' : '') + element[xsPrefix + 'annotation'][xsPrefix + 'documentation'];
      let desc = element[xsPrefix + 'annotation'][xsPrefix + 'documentation'];
      target.description = getDescription(desc, lang);
    }
    ...
    if (simpleType) {
      ...
      if (simpleType[xsPrefix + 'annotation'] && simpleType[xsPrefix + 'annotation'][xsPrefix + 'documentation']) {
        typeData.description = simpleType[xsPrefix + 'annotation'][xsPrefix + 'documentation']['#text'];
      }
    }
...
module.exports = {
  getJsonSchema: function getJsonSchema(src, title, outputAttrPrefix, laxURIs, newXsPrefix, lang) {
    // TODO convert to options parameter
    reset(outputAttrPrefix, laxURIs, newXsPrefix, lang);
    ...
    recurse(obj, {}, function (src, parent, key) {
      // was obj.properties
    ...
    var count = 1;
    while (count > 0) {
      // loop until we haven't removed any empties
      count = 0;
      recurse(obj, {}, function (obj, parent, key) {
